...
def getLeg1ThresholdForDoubleEle(year, hltTrigger='HLT_Ele23_Ele12_CaloIdL_TrackIdL_IsoVL', debug=False):
    ...
    def download(url, destination):
        ...
        try:
            ...
            pass
        urllib.request.urlretrieve(url, os.path.join(destination, url.split('/')[-1]))
    ...
    with open(os.path.join(dirToStore, hltTrigger + '.php')) as f:
        for line in f:
            if 'prescale1' in line and 'L1_DoubleEG' in line:
                download(prescalePage + line.split('>')
                         [0].split('=')[-1], dirToStore)

    jsonForThreshold = {}
    for json in glob.glob(os.path.join(dirToStore, '*.json')):
        leg1 = int(json.split('L1_DoubleEG_')
                   [-1].split('_')[0].replace('LooseIso', ''))
        leg2 = int(json.split('L1_DoubleEG_')
                   [-1].split('_')[1].replace('LooseIso', ''))
        if leg1 in jsonForThreshold:
            # this theshold already exists, so we merge them into the existing one
            mergeLumis(jsonForThreshold[leg1], json)
        else:
            jsonForThreshold[leg1] = json

    thresholdsToSubtract = []
    # sorting from low to high thresholds
    for threshold in sorted(jsonForThreshold.keys()):
        if (debug):
            print()
